...
@section('content')
    <h1 class="h3 mb-4 text-gray-800">Edit supervisor</h1>
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h4 class="m-0 font-weight-bold text-primary">Edit supervisor</h4>
        </div>
        <div class="card-body">
            <form action="{{route('admin.supervisor.update','test')}}" method="post" autocomplete="off"
    ...
    <script>
        $(function () {
            $("#image_updload").fileinput({
                ...
                initialPreview: [
                    @if($data->photo)
                        "{{asset('admin/pictures/supervisor/' . $data->id . '/'  . $data->photo->Filename)}}"
                    @endif
                ],
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('admin.supervisor_remove_image',['data_id' => $data->id,'photo_id' => $data->photo->id ,'photo_name' => $data->photo->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->photo->id}}
                    }
                ]
                @endif
